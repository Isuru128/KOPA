...
import com.kopa.repository.ProductRepository;
...
public class ProductService {
    ...
    private final Map<String, Product> fallbackMap = new ConcurrentHashMap<>();
    ...
    private void seedProducts() {
        ...
        products.forEach(p -> fallbackMap.put(p.getId(), p));

        try {
            if (productRepository.count() == 0) {
                productRepository.saveAll(products);
                log.info("Successfully seeded {} coffee shop products into MongoDB Atlas", products.size());
            }
        } catch (Exception e) {
            log.warn("Could not seed products to MongoDB Atlas (check MONGODB_URI in .env): {}", e.getMessage());
        }
    }
    ...
    public List<Product> getAllProducts() {
        try {
            ...
            log.warn("Reading products from fallback cache: {}", e.getMessage());
        }
    ...
    public Optional<Product> getProductById(String id) {
        ...
        try {
            ...
            log.warn("Reading product by ID from fallback cache: {}", e.getMessage());
        }
    ...
    public List<Product> getProductsByCategory(String category) {
        ...
        try {
            ...
            log.warn("Filtering products by category from fallback cache: {}", e.getMessage());
        }
    ...
    public Product addProduct(Product p) {
        fallbackMap.put(p.getId(), p);
        try {
            return productRepository.save(p);
        } catch (Exception e) {
            log.warn("Could not save product to MongoDB: {}", e.getMessage());
            return p;
        }
    }
}
